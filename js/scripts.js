...
(function ($) {
	...
	if (
		...
		$(".primary-nav.desktop, .primary-nav.mobile")
			.removeClass("d-none")
			.addClass("d-flex");
		$(".primary-nav.desktop").css("display", "flex !important");
		$(
			".primary-nav, .secondary-nav .currency.dropdown, .nav-group .search"
		).hide();
	...
	customQty.click(function () {
		...
		$(
			".custom-calculator-input-wrapper .discount-price .price i.calc-check"
		).removeClass("hide");
	});
	...
	$(".category-items .item").each(function () {
		const dropdownHeader = $(
			".selected-category .dropdown .btn .dropdown-title"
		);
	...
	// Toggle active class between prod-nav-links on custom stickers page
	$(".prod-nav-links li a").each(function () {
		$(this).on("click", function () {
			$(".prod-nav-links li a").each(function () {
				// $(this).removeClass("active");
			});
			// $(this).addClass("active");
			let eleId = `${$(this).attr("data-href")}`;
			let pos = $(`#${eleId}`).offset().top - 100;
			$("html, body").animate(
				{
					scrollTop: pos,
				},
	...
	// Toggle between nav-links while scrolling
	$(window).scroll(function () {
		$(".mini-wrapper .target-sec").each(function () {
			if ($(window).scrollTop() >= $(this).offset().top - 120) {
				var bId = `${$(this).attr("id")}`;
				// $(".prod-nav-links li a").removeClass("active");
				$('.prod-nav-links li a[data-href="' + bId + '"]')
					.addClass("active")
					.parent("li")
					.siblings("li")
					.find("a")
	...
	//Product info custom accordions
	$(".accordion_body").click(function () {
		$(this).parent().toggleClass("active");
	});
	$(".accordion_btn img").click(function () {
